...
// From June 8 2020 XXX




// XXX Additional details from discussions within the calorimetry working group:
// 
...
Smear::Detector BuildMatrixDetector_0_1() {
  ...
  // eta = -2 -- -1
  // stoch. = 7%
  // Could use const. = 1% (bit worse than KLOE) XXX
  //   // but using the Calo group's alternative instead, Shashlyk, Pb/Sc 8% stochastic, 2% constant.XXX
  ...
  // eta = -3.5 -- -1
  // stoch. = 50%
  // Using configuration 2 from calo group, Pb/Sc 45% stochastic, 6% constant terms. XXX
  Smear::Acceptance::Zone HcalBackZone(ThetaFromEta ( -1 ),ThetaFromEta ( -3.5 ));
  Smear::Device HcalBack(Smear::kE, "sqrt(pow( 0.00*E, 2) + pow ( 0.5,2) *E)");
  ...
  Smear::Device HcalFwd(Smear::kE, "sqrt(pow( 0.00*E, 2) + pow ( 0.5,2) *E)");
