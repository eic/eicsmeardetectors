...
#include "piddetectors/DircBarrelSmearer.h"
...
Smear::Detector BuildMatrixDetector_0_1_PID() {
  ...
  // trackResolution, timePrecision, qe, etaLow,  etaHigh  
  Smear::DircBarrelSmearer dircBarrel(0.5, -0.1, 0, -1.0, 1.0 );
  // Restricting to the tracker barrel defined above
  // but additional cuts are done by the detector!
  dircBarrel.Accept.AddZone( TrackBarrelZone ); 
  dircBarrel.Accept.SetCharge(Smear::kCharged);
  det.AddDevice(dircBarrel);
  
  // For diagnostic purposes, we will cheat here and also set the id to the truth value
  // Note that one shouldn't really use this field - use the more realistic, smeared, numSigma instead
  Smear::PerfectID barrelperfectpid;
  barrelperfectpid.Accept.AddZone(TrackBarrelZone);
  barrelperfectpid.Accept.SetCharge(Smear::kCharged);
  det.AddDevice( barrelperfectpid );
